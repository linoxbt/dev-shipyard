// Solana variant of Code with AI — rendered by /launchkit/ai when the active
// chain family is Solana. Reuses the language-agnostic AI transport (src/lib/ai)
// with a Rust/Anchor + SPL system prompt.
...
import { SOLANA_SYSTEM_PROMPT } from "@/lib/solana/ai-prompts";
import { cn } from "@/lib/utils";

const SUGGESTIONS = [
  "Write an Anchor program for a token vesting vault",
  "Explain PDAs and how to derive them safely",
  "Create an SPL token with a fixed supply and mint it to me",
  "Audit this Anchor program for account-substitution bugs",
];

export function SolanaAiView() {
  ...
  return (
    <div className="flex h-[calc(100vh-0px)] flex-col">
      <PageHeader
        ...
        title="Code with AI"
        subtitle="Anchor/Rust + SPL assistant. Generate programs, tokens, and client code."
        ...
        {messages.length === 0 ? (
          <div className="mx-auto max-w-2xl">
            <div className="mb-4 flex items-center gap-2 font-mono text-sm text-muted-foreground">
              <Sparkles className="h-4 w-4 text-primary" /> Ask anything about building on Solana.
            </div>
            {!configured && (
      ...
      <div className="border-t border-border p-4">
        <div className="mx-auto flex max-w-3xl items-end gap-2">
          <textarea
            ...
            rows={1}
            placeholder="Ask about Anchor, SPL tokens, PDAs…"
          ...
          {streaming ? (
            <button
              onClick={stop}
              className="inline-flex items-center gap-1 rounded border border-border px-3 py-2 font-mono text-sm text-danger hover:border-danger"
            >
