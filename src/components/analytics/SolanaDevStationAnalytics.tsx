...
const AMBER = "#f59e0b";
const KIND_COLOR: Record<string, string> = { token: "#14b8a6", nft: "#9945FF", program: "#f59e0b" };

function dayKey(ts: number): string {
  return new Date(ts * 1000).toISOString().slice(0, 10);
}

export function SolanaDevStationAnalytics({ cluster }: { cluster: SolanaCluster }) {
  const [all, setAll] = useState<SolanaDeploy[]>([]);

  useEffect(() => {
    const refresh = () => setAll(loadSolanaDeploys());
    refresh();
    window.addEventListener("devstation-solana-deploys", refresh);
    window.addEventListener("storage", refresh);
    return () => {
      window.removeEventListener("devstation-solana-deploys", refresh);
      window.removeEventListener("storage", refresh);
    };
  }, []);

  const deploys = useMemo(() => all.filter((d) => d.cluster === cluster), [all, cluster]);
  ...
  const { series, byType, kpis } = useMemo(() => {
    ...
    const programs = deploys.filter((d) => d.kind === "program").length;
    const byType = [
      { name: "Tokens", count: tokens, key: "token" },
      { name: "NFTs", count: nfts, key: "nft" },
      { name: "Programs", count: programs, key: "program" },
    ].filter((x) => x.count > 0);
    return {
      series,
      byType,
    ...
    return (
      <div className="p-4 lg:p-6">
        <div className="mx-auto max-w-lg rounded border border-border bg-surface p-6 text-center">
          ...
          >
            <Rocket className="h-3.5 w-3.5" /> Deploy on Solana
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6 p-4 lg:p-6">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Kpi icon={Cpu} label="Programs" value={kpis.programs.toLocaleString()} />
      </div>
        ...
        <div className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={series} margin={{ top: 8, right: 12, bottom: 0, left: 0 }}>
              <defs>
                <linearGradient id="solDepFill" x1="0" y1="0" x2="0" y2="1">
      ...
      <Card title="Recent deployments" subtitle={`${deploys.length} recorded on this cluster`}>
        ...
        {deploys.slice(0, 12).map((d) => (
          ...
          >
            <span className="min-w-0">
              <span className="block truncate text-foreground">{d.name || d.kind}</span>
              <span className="block truncate text-meta">{truncateAddress(d.address, 8, 8)}</span>
            </span>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 px-4 py-2 font-mono text-xs">
