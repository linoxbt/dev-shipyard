...
| `SPONSOR_PRIVATE_KEY` | Optional. A live, funded QIE mainnet wallet the running app tops visitors' own wallets up from (not a relayer — see [DEPLOYMENT.md](./DEPLOYMENT.md#sponsored-deploys-qie-mainnet)). Unlike `PRIVATE_KEY` above, this one *is* meant to be set on a host — but only if you understand the abuse model, it's closer to an open token faucet than a scoped gas payer. |
