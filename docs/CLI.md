...
| it seems slow to answer the first message in a very large folder                    | the first turn builds a search index for the agent. It skips caches such as `~/.cache` and `~/.bun` and gives up after 20,000 files or 10 seconds, then answers                                                            |
